...
def get_ipv4_address(spawn):
    spawn.sendline(u"ifconfig")
    spawn.expect(u"[A-Za-z0-9]{2}_[0-9]+:  inet4 ([0-9\.]+); netmask ([0-9\.]+); gw ([0-9\.]+)")
    return spawn.match.group(1)


def test_ipv4_send(board_group, application, env=None):
    ...
    with pexpect.spawnu("make", ["-C", application, "term"], env=env_sender,
        ...
        receiver_ip = get_ipv4_address(receiver)

        receiver.sendline(u"ip server start %d" % ipprot)
        # wait for neighbor discovery to be done
        time.sleep(5)
        print(u"ip send %s %d 01:23:45:67:89:ab:cd:ef" % (receiver_ip, ipprot))
        sender.sendline(u"ip send %s %d 01:23:45:67:89:ab:cd:ef" % (receiver_ip, ipprot))
        sender.expect_exact(u"Success: send 8 byte over IPv4 to %s (next header: %d)" %
                            (receiver_ip, ipprot))
        receiver.expect(u"00000000  01  23  45  67  89  AB  CD  EF")


def test_udpv4_send(board_group, application, env=None):
    env_sender = os.environ.copy()
    if env is not None:
        env_sender.update(env)
    env_sender.update(board_group.boards[0].to_env())
...
def test_tcpv4_send(board_group, application, env=None):
    ...
    with pexpect.spawnu("make", ["-C", application, "term"], env=env_client,
        ...
        port = random.randint(0x0000, 0xffff)
        server_ip = get_ipv4_address(server)
        client_ip = get_ipv4_address(client)
...
def test_triple_send(board_group, application, env=None):
    ...
    with pexpect.spawnu("make", ["-C", application, "term"], env=env_sender,
        ...
        ipprot = random.randint(0x00, 0xff)
        receiver_ip = get_ipv4_address(receiver)
        sender_ip = get_ipv4_address(sender)
        ...
        sender.expect_exact(u"Success: send 4 byte over IPv4 to %s (next header: %d)" %
                            (receiver_ip, ipprot))
...
if __name__ == "__main__":
    ...
    TestStrategy().execute([BoardGroup((
                            Board("native", "tap0"),
                            Board("native", "tap1")))],
                           [test_ipv4_send, test_udpv4_send, test_tcpv4_send,
                           test_triple_send])
