...
def get_results_and_output_from(fd):
    ...
    results_prefix = 'Testing for '
    output_prefix = 'Testing application '
    prev_results = False
    result = ['']
    output = StringIO()
    test_result = ""
    while 1:
        ...
        if not line:
            ...
            if (len(result) > 1) and ('success' in result[0] or 'failed' in result[0]):
                ...
                stdout.flush()
                test_result = ""
            if (len(result) > 0) and 'failed' in result[0]:
                stdout.write("\nError log:\n" + test_result + "\n")
                stdout.flush()
                test_result = ""
...
def test_all():
    ...
    for folder in sorted(app_folders):
        ...
        print('Testing all applications in: {}'.format(colorize_str(folder, Termcolor.blue)))
            ...
            try:
                ...
                if returncode == 0:
                    ...
                    if not failed_with_output:
                        print(colorize_str('\t\ttest error!', Termcolor.red))
