...
def setup_child(timeout=10, spawnclass=pexpect.spawnu, env=None, logfile=None):
    env = os.environ.copy()
    target = 'term'
    if 'TESTRUNNER_TARGET' in os.environ:
        target = os.environ['TESTRUNNER_TARGET']
    print("make " + target)
    child = pexpect.spawnu("make " + target, env=env, timeout=timeout, codec_errors='replace')
    child = spawnclass("make " + target, env=env, timeout=timeout, codec_errors='replace', echo=False)
